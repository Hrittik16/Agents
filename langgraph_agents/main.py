...
class MessageClassifier(BaseModel):
    '''
    This is a pydantic model that defines the structure of the message.
    '''
...
def classify_message(state: State):
    ...
    # This will create a version of our llm that will give a structured output
    classifier_llm = llm.with_structured_output(MessageClassifier) 

    # We describe how the classifier llm should classify the messages
...
def run_chatbot():
    ...
    while True:
        ...
        if state.get("messages") and len(state["messages"]) > 0:
            ...
            print(f"({agent_type}) Assistant: {last_message.content}")
